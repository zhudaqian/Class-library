...
var Effect = {
	...
	Bounce: {//指数衰减的反弹缓动。
		...
		easeInOut: function(t,b,c,d){
			if (t < d/2) return Effect.Bounce.easeIn(t*2, 0, c, d) * .5 + b;
			else return Effect.Bounce.easeOut(t*2-d, 0, c, d) * .5 + c*.5 + b;
		}
	}
};
//Effect.Bounce.easeInOut//反弹的 5
//Effect.Elastic.easeInOut//弹性的flex 4
//Effect.Back.easeInOut//返回的 3
//Effect.Linear//匀速的，线性的 1
//Effect.Quart.easeOut; 2
function move(ele,obj,duration,effect,callback){
	//如果用JS实现方法重载
	if(typeof duration =="undefined"){	duration =600;	}
	var fnEffect=Effect.Quart.easeOut;//默认的动画效果
	switch(effect){
		case 1:
		...
		case 4:
			...
			break;
		case 5:
			fnEffect=Effect.Bounce.easeOut;
			break;
